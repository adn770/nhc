...
pub(super) fn draw(
    ...
    match op.kind() {
        HatchKind::Corridor => draw_corridor(&op, painter),
        HatchKind::Room => draw_room(&op, painter),
        _ => {}
    }
}

fn draw_corridor(op: &HatchOp<'_>, painter: &mut dyn Painter) {
...
fn draw_room(op: &HatchOp<'_>, painter: &mut dyn Painter) {
    let tiles: Vec<(i32, i32)> = match op.tiles() {
        Some(t) => t.iter().map(|c| (c.x(), c.y())).collect(),
        None => return,
    };
