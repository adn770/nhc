...
# Canonical background colour per biome -- the single source of
# truth for "what shade does this biome read as".
#
# The five full-palette biomes carry the RGB mode sampled from
# the bottom band (rows 150-190) of their slot-11 village tile,
# where the foundation feature doesn't cover the base hex. Those
# entries aren't generated by this tool (they ship as hand-drawn
# PNGs under hextiles/<biome>/), but keeping them declared here
# lets the rest of the codebase reference a single table -- for
# new features, fallback rendering, Biome enum palette
# harmonisation, or any future generator that needs to know how
# a biome "looks".
#
# The five generated biomes (forest, mountain, hills, marsh,
# swamp) also appear in GENERATED_BIOMES below; the main loop
# composes their tiles from the foundation overlay + this flat
# background. Their colours are chosen to sit in the same
# saturation band as the shipped palette so the generated tiles
# visually belong with the hand-drawn ones.
BIOME_COLOURS = {
    # --- Full-palette biomes (ship as shipped hand-drawn PNGs).
    # Values sampled from hextiles/<biome>/11-*_village.png,
    # bottom-band mode. Not regenerated by this tool; declared
    # here for reference.
    "greenlands": RGB(136, 184,  96),   # bright grass
    "drylands":   RGB(168, 128, 104),   # dusty tan
    "sandlands":  RGB(240, 208, 120),   # pale sand
    "icelands":   RGB(184, 200, 208),   # pale blue-grey
    "deadlands":  RGB( 80,  80,  64),   # dark olive-grey
    # --- Generated biomes (this tool composes their palettes
    # from foundation tiles + background fill).
...
# to regenerate its tiles from the foundation overlay, which
# would overwrite the shipped hand-drawn palette -- don't do
