...
map:
  ...
  continental:
    ...
    plate_count: 5
    # ... see design/hexcrawl_generator.md for full params
features:
  hub: 1
  village: { min: 2, max: 4 }
...
- Rivers originate at mountains/hills, flow downhill toward the
  coast, and may create lakes at low-elevation convergence
  points. They can cross forest hexes. Rivers die in drylands
  and sandlands.
- Settlements prefer river-adjacent and biome-border hexes.
- Roads connect settlements via terrain-aware A*, avoiding
  sandlands and deadlands. Dead-end roads get towers or keeps.
  Caves connect to the nearest road.
- Movement cost is unaffected by rivers/roads at the macro
  level; sub-hex roads halve the movement cost at flower level.

**Rendering:** Edge segments are drawn on the hex-feature-canvas
as Catmull-Rom spline curves. Rivers render as blue solid lines;
roads as brown dashed lines. At junction hexes (3+ roads
meeting), a fallback Bezier is used instead of flower sub-paths
for clean crossroads.

Generation algorithms are documented in
`design/hexcrawl_generator.md` (Stages 6-9).
...
## 7. World Map Generator

The overland map is generated by the **continental** pipeline,
a nine-stage process that builds terrain geologically: simplex
continental shape, Voronoi tectonic plates, domain warping,
flow-accumulation erosion, latitude-aware biome assignment,
drainage-based rivers, scored settlement placement, terrain-
aware roads, and edge-point continuity.
...
- `design/hexcrawl_generator.md` -- world map generator pipeline:
  continental shape, tectonic plates, erosion, biome assignment,
  rivers, settlements, roads, and edge-point continuity.
- `design/design.md` -- master architecture; authoritative for the
  dungeon layer that hex mode wraps.
- `design/dungeon_generator.md` -- BSP dungeon pipeline for
  mini-dungeon encounter maps.
