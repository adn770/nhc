...
from nhc.web.debug_bundle import build_debug_tar, slug_player_name
...
def create_app(
    ...
    *auth_token* is the **admin** token.  Player tokens are managed
    ...
    def _build_debug_tar(session, extra_entries=()) -> bytes:
        """Build a debug tar.gz for ``session`` (see debug_bundle)."""
        return build_debug_tar(
            session,
    ...
    def submit_report(session_id: str):
        ...
        slug = slug_player_name(player_name)
