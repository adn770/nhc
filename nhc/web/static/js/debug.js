...
const DebugPanel = {
  ...
  _buildExportTab() {
    ...
    // ── Layer PNG export ──
    frag.appendChild(this._sectionHeader("Layer PNGs"));

    const dungeonLayers = [
      { label: "Floor SVG",     el: "#floor-svg svg" },
      { label: "Door Canvas",   el: "#door-canvas" },
      { label: "Hatch Canvas",  el: "#hatch-canvas" },
      { label: "Fog Canvas",    el: "#fog-canvas" },
      { label: "Entity Canvas", el: "#entity-canvas" },
      { label: "Debug Canvas",  el: "#debug-canvas" },
    ];
    const hexLayers = [
      { label: "Hex Base",    el: "#hex-base-canvas" },
      { label: "Hex Feature", el: "#hex-feature-canvas" },
      { label: "Hex Fog",     el: "#hex-fog-canvas" },
      { label: "Hex Entity",  el: "#hex-entity-canvas" },
      { label: "Hex Debug",   el: "#hex-debug-canvas" },
    ];
    ...
    const makeLayerBtn = ({label, el}) => {
      const btn = document.createElement("button");
      btn.className = "debug-export-btn";
      btn.textContent = label;
      btn.addEventListener("click", () => {
        const target = document.querySelector(el);
        if (!target) { btn.textContent = "not found"; return; }
        const fname = label.toLowerCase().replace(/\s+/g, "_") + ".png";
        if (target.tagName === "svg") {
          svgToPng(target, fname);
        } else {
          downloadPng(target, fname);
        }
      });
      const row = document.createElement("div");
    ...
    dungeonLayers.forEach(l => frag.appendChild(makeLayerBtn(l)));
    hexLayers.forEach(l => frag.appendChild(makeLayerBtn(l)));

    // Export All Layers
    const allLayersBtn = document.createElement("button");
    allLayersBtn.className = "debug-export-btn";
    allLayersBtn.textContent = "Export All Layers";
    allLayersBtn.style.marginTop = "8px";
    allLayersBtn.style.borderColor = "#e6c07b";
    allLayersBtn.style.color = "#e6c07b";
    allLayersBtn.addEventListener("click", () => {
      const all = [...dungeonLayers, ...hexLayers];
      let count = 0;
      all.forEach(({label, el}) => {
        const target = document.querySelector(el);
        if (!target) return;
        // Skip canvases with zero dimensions (inactive mode).
        if (target.tagName === "CANVAS"
            && (!target.width || !target.height)) return;
        const fname = label.toLowerCase().replace(/\s+/g, "_") + ".png";
        if (target.tagName === "svg") {
          svgToPng(target, fname);
        } else {
      ...
      allLayersBtn.textContent = `Exported ${count} layers`;
      setTimeout(() => {
        allLayersBtn.textContent = "Export All Layers";
      }, 3000);
    });
    frag.appendChild(allLayersBtn);
