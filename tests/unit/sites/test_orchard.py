"""Orchard site assembler — inhabited-settlement family.

Replaces the ORCHARD branch of the retired
``generate_inhabited_settlement_site`` (M4f of sites-unification).
The orchard is a walled FIELD plaza with a central ``tree`` and a
3x3 grid of scattered ``tree`` tiles around it.
"""
...
class TestOrchardFeatureTiles:
    def test_has_multiple_tree_features(self):
        """Orchard is rows of trees, not a single tree."""
        site = assemble_orchard(
            "o1", random.Random(1),
            feature=MinorFeatureType.ORCHARD,
            tier=SiteTier.MEDIUM,
        )
        trees = _feature_tiles(site.surface, "tree")
        assert len(trees) >= 4

    def test_center_tree_is_walkable(self):
        site = assemble_orchard(
            "o1", random.Random(2),
            feature=MinorFeatureType.ORCHARD,
            tier=SiteTier.MEDIUM,
...
class TestOrchardSurface:
    ...
    def test_no_interior_walls(self):
        """The orchard is a clearing; only the 1-tile perimeter
        carries walls."""
        site = assemble_orchard(
            "o1", random.Random(1),
            feature=MinorFeatureType.ORCHARD,
...
class TestOrchardDeterminism:
    def test_same_seed_same_tree_grid(self):
        a = assemble_orchard(
            "o", random.Random(99),
            feature=MinorFeatureType.ORCHARD,
            tier=SiteTier.MEDIUM,
        ...
        assert (
            _feature_tiles(a.surface, "tree")
            == _feature_tiles(b.surface, "tree")
        )
